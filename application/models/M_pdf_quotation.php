...
class M_pdf_quotation extends CI_Model{
    ...
    function selectPerusahaan($where){
        return $this->db->query("select * from price_request join quotation on price_request.id_request = quotation.id_request join perusahaan on perusahaan.id_perusahaan = price_request.id_perusahaan WHERE quotation.id_submit_quotation = " . $where['id_submit_quotation']);
    }
