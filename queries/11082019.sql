...
select * from detail_finished_order;
...
price_request_item.nama_produk,
price_request_item.jumlah_produk,
price_request_item.satuan_produk,
price_request_item.notes_produk,
...
order_confirmation_item.satuan_produk,
...
/*inner join ke oc dan ke harga2 vendor, dkk*/

select * from quotation_metode_pembayaran inner join order_detail on order_detail.id_submit_quotation = quotation_metode_pembayaran.id_submit_quotation;

show columns from order_detail;

show columns from price_request;
...
inner join perusahaan
on perusahaan.id_perusahaan = contact_person.id_perusahaan 
order by date_cp_add DESC;
...
select * from harga_courier;
select * from harga_shipping;
